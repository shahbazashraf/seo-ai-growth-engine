...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const redirectUri = Deno.env.get("GOOGLE_GSC_REDIRECT_URI")?.trim() || Deno.env.get("GOOGLE_REDIRECT_URI")?.trim() || "";
    ...
    if (pathname.endsWith("/callback")) {
      ...
      await saveToken(stateRecord.user_id, "google-search-console", {
      ...
      return Response.redirect(`${siteUrl}/settings?connected=gsc`, 302);
    }
